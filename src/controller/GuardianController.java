...
public class GuardianController implements Initializable,MainControllerAware,DataReceiver {
    ...
    void BtnAddOnAction(ActionEvent event) throws NoSuchAlgorithmException, Exception {
        if (TextName.getText().isEmpty() || TextLast_name.getText().isEmpty()
            ...
            showAlert("Error", "Todos los campos del primer tutor deben ser llenados.", Alert.AlertType.WARNING);
            return;
        }else if(TextTelf_home.getText().isEmpty() && TextTelf_personal.getText().isEmpty() && TextTelf_work.getText().isEmpty()){
                    showAlert("Error", "Debe Llenar un telefono como minimo.", Alert.AlertType.WARNING);
                    return;
        ...
        //Segundo Tutor
            ...
            try {
                ...
                if (tutorId1 > 0) {
                    ...
                    if (!TextTelf_work.getText().isEmpty()) {
                        if(VerifyNumberUser(TextTelf_work.getText())){
                            ...
                            if (rsp) {
                                ...
                                alert.setHeaderText(null);
                                alert.setContentText("Se registro correctamente el tutor");
                                alert.initStyle(StageStyle.UTILITY);
                                alert.showAndWait();
                            } else {
                    ...
                    if (tutorId1 > 0) {
                        if (tutorId2 > 0) {
                            showAlert("Éxito", "Tutores registrado correctamente.", Alert.AlertType.INFORMATION);                        
                        }else{
                            showAlert("Éxito", "Tutor registrado correctamente.", Alert.AlertType.INFORMATION);                        
                        }
                    }
                    ...
                    showAlert("Error", "No se pudo registrar el tutor.", Alert.AlertType.ERROR);
                }
            } catch (Exception e) {
                e.printStackTrace();
                showAlert("Error", "Ocurrió un error al registrar el tutor.", Alert.AlertType.ERROR);
            }
        }
    }
    @FXML
    void onEdit(ActionEvent event) throws NoSuchAlgorithmException, Exception{
        if (tutor1 != null) {
            if (TextName.getText().isEmpty() || TextLast_name.getText().isEmpty()
                ...
                alert.setHeaderText(null);
                alert.setContentText("Todos los campos del tutor 1 deben estar completos.");
                alert.initStyle(StageStyle.UTILITY);
                alert.showAndWait();
                return;
            }
        ...
        if (tutor2 != null) {
            if (TextName1.getText().isEmpty() || TextLast_name1.getText().isEmpty()
                ...
                alert.setTitle("Advertencia");
                alert.setHeaderText(null);
                alert.setContentText("Todos los campos del tutor 2 deben estar completos.");
                alert.initStyle(StageStyle.UTILITY);
                alert.showAndWait();
                return;
            }
        ...
        alert.setHeaderText(null);
        alert.setContentText("Los tutores han sido modificados con éxito.");
        alert.showAndWait();
    }
    ...
    private void fillGuardianFields( boolean isSecond) {
        if (tutor1 != null) {
            ...
            for (Telf_Guardian tel : telefonos1) {
                switch (tel.getTipo()) {
                    case 1: TextTelf_personal.setText(tel.getNumero()); break;
                    case 2: TextTelf_home.setText(tel.getNumero()); break;
                    case 3: TextTelf_work.setText(tel.getNumero()); break;
                }
            }
        }
        ...
        if (tutor2 != null) {
            ...
            for (Telf_Guardian tel : telefonos2) {
                switch (tel.getTipo()) {
                    case 1: TextTelf_personal1.setText(tel.getNumero()); break;
                    case 2: TextTelf_home1.setText(tel.getNumero()); break;
                    case 3: TextTelf_work1.setText(tel.getNumero()); break;
                }
            }
        }
    }
    private void fillPhoneFields(List<Telf_Guardian> telefonos, boolean isSecond) {
        for (Telf_Guardian telf : telefonos) {
            switch (telf.getTipo()) {
                case 3:
                    if (!isSecond) TextTelf_personal.setText(telf.getNumero());
                    else TextTelf_personal1.setText(telf.getNumero());
                    break;
                case 2:
                    if (!isSecond) TextTelf_home.setText(telf.getNumero());
                    else TextTelf_home1.setText(telf.getNumero());
                    break;
                case 1:
                    if (!isSecond) TextTelf_work.setText(telf.getNumero());
                    else TextTelf_work1.setText(telf.getNumero());
                    break;
            }
        }
    }
